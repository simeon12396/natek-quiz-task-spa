...
import { RadioGroup, FormControlLabel, Radio, Typography, CardActions, Paper } from "@material-ui/core";
...
const SkeletonQuiz = (props) => {
  ...
  const handleClickSubmit = () => {
    ...
    const answerResults = correctAnswers.map((c, index) =>
      ...
      <Paper elevation={3}>
        ...
        <Paper elevation={0} className={styles.contentQuiz}>
          <div>
            ...
            {quizMode === "multiple" && <Typography variant="body2">{multipleChoicesQuestions.results[questionCounter].question}</Typography>}
          </div>
          <div>
            <RadioGroup name="quizOptions" value={selectedCurrentAnswers} onChange={handleChangeOption} classes={{ root: styles.radioGruoupOptionRoot }}>
              {quizMode === "binary" && booleanAnswerOptions.map((b) => <FormControlLabel value={b} control={<Radio classes={{ checked: styles.radioChecked }} />} label={b} key={b} />)}
              {quizMode === "multiple" && multipleChoicesAnswerOptions.map((m) => <FormControlLabel value={m} control={<Radio classes={{ checked: styles.radioChecked }} />} label={m} key={m} />)}
            </RadioGroup>
          </div>
        </Paper>
        <CardActions classes={{ root: styles.cardActionsRoot }}>
          <CustomButton disabled={questionCounter === 9 ? true : false} onClick={handleClickNext}>
            Next question
          </CustomButton>

          {questionCounter === 9 && (
            <CustomButton onClick={handleClickSubmit} disabled={isQuizSubmitted}>
              Submit
            </CustomButton>
          )}

          {isQuizSubmitted && (
            <CustomButton variant="contained" color="primary" className={styles.restartButton} onClick={handleClickRestart}>
              Restart
            </CustomButton>
          )}
          ...
          {isQuizSubmitted && (
            <CustomButton variant="contained" color="primary">
              <NavLink to={`/quiz/${quizType === "music" ? "music" : "computer-science"}-result`} className={styles.link}>
                View Result
              </NavLink>
            </CustomButton>
          )}
